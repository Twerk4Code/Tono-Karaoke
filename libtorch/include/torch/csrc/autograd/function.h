...
struct TORCH_API Node : std::enable_shared_from_this<Node> {
 public:
  ...
  uint32_t add_input_metadata(
    ...
    uint32_t input_nr = c10::checked_convert<uint32_t>(
        input_metadata_.size(), "input_metadata_.size()");
  ...
  uint32_t num_inputs() const noexcept {
    return c10::checked_convert<uint32_t>(
        input_metadata_.size(), "input_metadata_.size()");
  }
  ...
  uint32_t num_outputs() const noexcept {
    return c10::checked_convert<uint32_t>(
        next_edges_.size(), "next_edges_.size()");
  }
